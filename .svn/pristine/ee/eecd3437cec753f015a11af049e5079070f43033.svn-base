...
dhtmlXGridObject.prototype._pgn_toolbar = function(page, start, end) {
    ...
    if (this.rC[2]) {
        ...
        //this.bk.forEachListOption("pages", function(id) {
         //           that.bk.removeListOption("pages", id)
          //      });
        $("#"+this.pagesoldid).empty();
        for (var i = 0; i < Cg; i++) {
            //this.bk.addListOption('pages', 'pages_' + (i + 1), NaN, "button", this.i18n.paging.page + (i + 1))
            $("#"+this.pagesoldid).append("<option value='"+(i + 1)+"'>第" + (i + 1)+"页</option>");
        }
        $("#"+this.pagesoldid).val(page);
    ...
    if (this.rC[3]){
        ...
        $("#"+this.perpagenumoldid).val(this.rowsBufferOutSize.toString());
    }
    this.callEvent("onPaging", [])
};
dhtmlXGridObject.prototype._pgn_createToolBar = function() {
    ...
    if (this.rC[2]) {
        ...
        this.pagesoldid ="pagesold_"+new Date().getTime();
        this.bk.addText("pagesold", NaN, "<select class='pages' id='"+this.pagesoldid+"'></select>");
        $("#"+this.pagesoldid).bind("change",function(){
    ...
    if (this.rC[3]) {
        ...
        this.perpagenumoldid ="perpagenumold_"+new Date().getTime();
        this.bk.addText("perpagenumold", NaN, "<select class='perpagenum' id='"+this.perpagenumoldid+"'></select>");
        for (var k = 5; k <= 50; k += 5){
            //this.bk.addListOption('perpagenum', 'perpagenum_' + k, NaN, "button", k + " " + this.i18n.paging.perpage);
            $("#"+this.perpagenumoldid).append("<option value='"+k+"'>每页"+ k + "行</option>");
        }
        $("#"+this.perpagenumoldid).bind("change",function(){
            self.rowsBufferOutSize = parseInt($(this).val());
            self.changePage();
        });
    };
