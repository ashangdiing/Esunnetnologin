...
dhtmlXGridObject.prototype._process_json = function(data) {
    ...
    if (!this.$__setHeader || data.total_count == 0) {
        if (this.$__setHeader) {
            this.clearAll(true);
        }
        if (data.Header) {
        ...
        if (data.ColAlign) {
            this.setColAlign(data.ColAlign);
        }
        if (data.ColTypes) {
            this.setColTypes(data.ColTypes);
        }
        if (data.ColSorting) {
            this.setColSorting(data.ColSorting);
        }
        if (data.Width) {
            this.setInitWidths(data.Width);
        }
        this.init();
        //this.init();
        this.$__setHeader = true;
    }
